...
where ẑ_t is the posterior median of the common growth factor. The dependent variable r_{t+1}^e is the U.S. market excess return (Rm−Rf) from the [Kenneth R. French Data Library](https://mba.tuck.dartmouth.edu/pages/faculty/ken.french/data_library.html). The sign and magnitude of β across regimes — negative early in recessions, strongly positive thereafter — is the paper's central empirical finding.
